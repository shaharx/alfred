...
function setLicense(licenseName, targetPath) {
    var licensePath = `${directories.licenses}/${licenseName}`
    try {
        fs.copyFileSync(licensePath, `${targetPath}/${im.getPath().license}`)
        ls.success(`${licenseName} license was added to ${targetPath}`)
    } catch (e) {
        ls.error(`There was and issue with adding the license, please verify that ${licenseName} exists under ${directories.licenses}`)
    }
}
